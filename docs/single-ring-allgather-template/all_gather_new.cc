...
HcclResult AllGatherNew::Tx(const LINK &link, const Slice &txSlice)
{
    // 单个发送示例
    /*
    DeviceMem srcMem = outputMem_.range(txSlice.offset, txSlice.size);
...
HcclResult AllGatherNew::Rx(const LINK &link, const Slice &rxSlice)
{
    /*
    DeviceMem dstMem = outputMem_.range(rxSlice.offset, rxSlice.size);
    HCCL_DEBUG("rx dstMem[%p] range[%llu], size[%llu] ",  dstMem.ptr(),
...
{
    // RunAsync方法实现了算法的具体执行逻辑 New算法的实现示例
    /*
...
//这是最终执行数据发送的方法，通过link的方法进行数据的发送和接收
...
// CHK_RET(links[i]->PostFinAck(param.stream)); CHK_RET(links[i]->WaitFinAck(param.stream)); 
// CHK_RET(links[i]->TxDataSignal(param.stream)); CHK_RET(links[i]->RxDataSignal(param.stream));