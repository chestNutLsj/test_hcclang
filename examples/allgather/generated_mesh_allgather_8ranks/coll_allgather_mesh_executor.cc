...
#include "coll_allgather_mesh_executor.h"

namespace hccl {
AllgatherMesh::AllgatherMesh(const HcclDispatcher dispatcher, std::unique_ptr<TopoMatcher> &topoMatcher)
    : CollAllgatherExecutor(dispatcher, topoMatcher)
{
    DMAReduceFlag_ = false;
}

HcclResult AllgatherMesh::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[AllgatherMesh][CalcStreamNum] tag[%s] streamNum_[%u]",
        tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}

HcclResult AllgatherMesh::CalcCommInfo(std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult AllgatherMesh::CalcLevel0CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
    // Mesh topology communication setup
    CommParaInfo commParaLevel0(COMM_LEVEL0, CommType::COMM_TAG_MESH);
    CHK_RET(CalcCommPlaneInfo(tag_, commParaLevel0, opTransport[COMM_LEVEL0], inputType, outputType));
    return HCCL_SUCCESS;
}

HcclResult AllgatherMesh::CalcLevel1CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult AllgatherMesh::CalcLevel2CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
    if( algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_AHC ||
        algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_AHC_BROKE) {
        HCCL_INFO("[AllgatherMesh][CalcLevel2CommInfo] select AHC bypass level2 comm calculate");
        return HCCL_SUCCESS;
    }
...
void AllgatherMesh::ParseParam(const OpParam& param)
{
    tag_ = param.tag;
    root_ = param.root;
...
HcclResult AllgatherMesh::CalcTransportMemType(TransportMemType &inputType, TransportMemType &outputType)
{
    if (workflowMode_ == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE) {
    ...
    HCCL_INFO("[AllgatherMesh][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d]",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

u64 AllgatherMesh::CalcLoopMaxCount(const u64 cclBuffSize, const u32 unitSize)
{
    u64 maxCountPerLoop = cclBuffSize / (topoAttr_.userRankSize * unitSize);
    return maxCountPerLoop;
}

bool AllgatherMesh::IsDataSplitForRdmaSdmaConcurrent(const u64 curSize)
{
...
HcclResult AllgatherMesh::KernelRun(const OpParam &param, ExecMem &execMem)
{
    u32 perDataSize = 0;
    CHK_RET(SalGetDataTypeSize(param.DataDes.dataType, perDataSize));
    CHK_PRT_RET(perDataSize == 0,
        HCCL_ERROR("[AllgatherMesh][KernelRun]errNo[0x%016llx] datatype[%d] is invalid",
    ...
    std::unique_ptr<AlgTemplateBase> algorithmTemplate = 
        AlgTemplateRegistry::Instance().GetAlgTemplate(
            TemplateType::TEMPLATE_ALLGATHER_MESH, dispatcher_);
    ...
    HCCL_INFO("[AllgatherMesh][KernelRun] Algorithm execution completed.");
    return HCCL_SUCCESS;
}

HcclResult AllgatherMesh::KernelRunInterServer(const OpParam &param, ExecMem &execMem)
{
    HCCL_INFO("[AllgatherMesh][KernelRunInterServer] Delegating to KernelRun");
    return KernelRun(param, execMem);
    return HCCL_SUCCESS;
}

HcclResult AllgatherMesh::KernelRunIntraServer(const OpParam &param, ExecMem &execMem)
{
    HCCL_INFO("[AllgatherMesh][KernelRunIntraServer] Delegating to KernelRun");
    return KernelRun(param, execMem);
    return HCCL_SUCCESS;
}

HcclResult AllgatherMesh::Orchestrate(const OpParam& param, AlgResourceResponse& algRes)
{
    HCCL_INFO("[AllgatherMesh][Orchestrate] Starting algorithm orchestration");
...
REGISTER_EXEC("AllgatherMeshExecutor", AllgatherMesh, AllgatherMesh);
