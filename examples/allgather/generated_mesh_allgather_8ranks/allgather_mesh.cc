...
HcclResult AllgatherMesh::RunAllgather(u32 rank, u32 rankSize, const std::vector<Slice> &outputSlices, const std::vector<LINK> &links)
{
    ...
    // Mesh AllGather with parallel communication pattern
    // DSL-driven fully connected topology - all ranks communicate with all others
    ...
    for (u32 peerRank = 0; peerRank < rankSize; peerRank++) {
            ...
            // Memory setup for bidirectional communication
            DeviceMem srcMem = outputMem_.range(rank * chunkSize, chunkSize);     // Own data to send
            DeviceMem dstMem = outputMem_.range(peerRank * chunkSize, chunkSize); // Buffer for peer's data
            ...
            // Launch async send (own data to peer)
            CHK_RET(links[peerRank]->TxAsync(UserMemType::OUTPUT_MEM,
                rank * chunkSize + baseOffset_, srcMem.ptr(), chunkSize, stream_));
        
            // Launch async receive (peer's data)
            CHK_RET(links[peerRank]->RxAsync(UserMemType::OUTPUT_MEM,
                peerRank * chunkSize + baseOffset_, dstMem.ptr(), chunkSize, stream_));
    }
