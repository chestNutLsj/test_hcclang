...
HcclResult AllgatherRecursiveDoubling::RunAllgather(u32 rank, u32 rankSize, const std::vector<Slice> &outputSlices, const std::vector<LINK> &links)
{
    ...
    // Loop analysis: {'has_loops': True, 'loop_type': 'recursive_doubling', 'iteration_variable': None, 'loop_condition': None, 'loop_increment': None, 'loop_body_operations': [], 'peer_calculation_pattern': 'xor', 'data_exchange_pattern': None, 'iteration_count': 4}
    ...
    for (u32 step = 0; step < 4; step++) {
        ...
        if (peer >= links.size()) {
            HCCL_ERROR("[AllgatherMesh][Loop] peer[%u] >= linkSize[%zu]", peer, links.size());
            return HCCL_E_INTERNAL;
        }
        ...
        if (rank < peer) {
                ...
                CHK_RET(links[peer]->TxAck(stream_));  // Signal: I'm ready to receive
                CHK_RET(links[peer]->RxAck(stream_));  // Wait: peer is ready to send
        } else {
                // Higher rank ID: respond to handshake
                CHK_RET(links[peer]->RxAck(stream_));  // Wait: peer initiates handshake
                CHK_RET(links[peer]->TxAck(stream_));  // Response: I'm ready to send
        }
        
        // Recursive doubling data exchange for step
        u32 exchangeSize = 1 << step;  // 2^step elements to exchange
        
        // Calculate data ranges based on recursive doubling algorithm
        u32 myGroupStart = (rank / exchangeSize) * exchangeSize;
        u32 peerGroupStart = (peer / exchangeSize) * exchangeSize;
        
        // Send data from our group that peer needs
        for (u32 sendRank = myGroupStart; sendRank < myGroupStart + exchangeSize; sendRank++) {
                if (sendRank < rankSize) {
                        Slice sendSlice = outputSlices[sendRank];
                        CHK_RET(Tx(links[peer], sendSlice));
                }
        }
        
        // Receive data from peer's group
        for (u32 recvRank = peerGroupStart; recvRank < peerGroupStart + exchangeSize; recvRank++) {
                if (recvRank < rankSize) {
