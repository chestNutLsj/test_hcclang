...
#include "coll_allgather_ring_executor.h"

namespace hccl {
AllgatherRing::AllgatherRing(const HcclDispatcher dispatcher, std::unique_ptr<TopoMatcher> &topoMatcher)
    : CollAllgatherExecutor(dispatcher, topoMatcher)
{
    DMAReduceFlag_ = false;
}

HcclResult AllgatherRing::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[AllgatherRing][CalcStreamNum] tag[%s] streamNum_[%u]",
        tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}

HcclResult AllgatherRing::CalcCommInfo(std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult AllgatherRing::CalcLevel0CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
    // Ring topology communication setup
...
HcclResult AllgatherRing::CalcLevel1CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult AllgatherRing::CalcLevel2CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
    if( algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_AHC ||
        algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_AHC_BROKE) {
        HCCL_INFO("[AllgatherRing][CalcLevel2CommInfo] select AHC bypass level2 comm calculate");
        return HCCL_SUCCESS;
    }
...
void AllgatherRing::ParseParam(const OpParam& param)
{
    tag_ = param.tag;
    root_ = param.root;
...
HcclResult AllgatherRing::CalcTransportMemType(TransportMemType &inputType, TransportMemType &outputType)
{
    if (workflowMode_ == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE) {
    ...
    HCCL_INFO("[AllgatherRing][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d]",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

u64 AllgatherRing::CalcLoopMaxCount(const u64 cclBuffSize, const u32 unitSize)
{
    u64 maxCountPerLoop = cclBuffSize / (topoAttr_.userRankSize * unitSize);
    return maxCountPerLoop;
}

bool AllgatherRing::IsDataSplitForRdmaSdmaConcurrent(const u64 curSize)
{
...
HcclResult AllgatherRing::KernelRun(const OpParam &param, ExecMem &execMem)
{
    u32 perDataSize = 0;
    CHK_RET(SalGetDataTypeSize(param.DataDes.dataType, perDataSize));
    CHK_PRT_RET(perDataSize == 0,
        HCCL_ERROR("[AllgatherRing][KernelRun]errNo[0x%016llx] datatype[%d] is invalid",
    ...
    std::unique_ptr<AlgTemplateBase> algorithmTemplate = 
        AlgTemplateRegistry::Instance().GetAlgTemplate(
            TemplateType::TEMPLATE_ALLGATHER_RING, dispatcher_);
    ...
    HCCL_INFO("[AllgatherRing][KernelRun] Algorithm execution completed.");
    return HCCL_SUCCESS;
}

HcclResult AllgatherRing::KernelRunInterServer(const OpParam &param, ExecMem &execMem)
{
    HCCL_INFO("[AllgatherRing][KernelRunInterServer] Delegating to KernelRun");
    return KernelRun(param, execMem);
    return HCCL_SUCCESS;
}

HcclResult AllgatherRing::KernelRunIntraServer(const OpParam &param, ExecMem &execMem)
{
    HCCL_INFO("[AllgatherRing][KernelRunIntraServer] Delegating to KernelRun");
    return KernelRun(param, execMem);
    return HCCL_SUCCESS;
}

HcclResult AllgatherRing::Orchestrate(const OpParam& param, AlgResourceResponse& algRes)
{
    HCCL_INFO("[AllgatherRing][Orchestrate] Starting algorithm orchestration");
...
REGISTER_EXEC("AllgatherRingExecutor", AllgatherRing, AllgatherRing);
