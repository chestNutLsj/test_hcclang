...
#ifndef ALLTOALL_MESH_H_PUB_H
#define ALLTOALL_MESH_H_PUB_H

#include "alg_template_base_pub.h"

namespace hccl {
class AlltoallMesh : public AlgTemplateBase {
public:
    explicit AlltoallMesh(const HcclDispatcher dispatcher);
    ~AlltoallMesh() override;

    HcclResult RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK> &links) override;

private:
    // Core algorithm implementation
    HcclResult RunAlltoall(u32 rank, u32 rankSize, const std::vector<Slice> &outputSlices, const std::vector<LINK> &links);
