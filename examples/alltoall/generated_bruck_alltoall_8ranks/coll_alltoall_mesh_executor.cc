...
#include "coll_alltoall_mesh_executor.h"

namespace hccl {
AlltoallMesh::AlltoallMesh(const HcclDispatcher dispatcher, std::unique_ptr<TopoMatcher> &topoMatcher)
    : CollAlltoallExecutor(dispatcher, topoMatcher)
{
    DMAReduceFlag_ = false;
}

HcclResult AlltoallMesh::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[AlltoallMesh][CalcStreamNum] tag[%s] streamNum_[%u]",
        tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}

HcclResult AlltoallMesh::CalcCommInfo(std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult AlltoallMesh::CalcLevel0CommInfo(TransportMemType inputType, TransportMemType outputType,
...
HcclResult AlltoallMesh::CalcLevel1CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult AlltoallMesh::CalcLevel2CommInfo(TransportMemType inputType, TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
    if( algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_AHC ||
        algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_AHC_BROKE) {
        HCCL_INFO("[AlltoallMesh][CalcLevel2CommInfo] select AHC bypass level2 comm calculate");
        return HCCL_SUCCESS;
    }
...
void AlltoallMesh::ParseParam(const OpParam& param)
{
    tag_ = param.tag;
    root_ = param.root;
...
HcclResult AlltoallMesh::CalcTransportMemType(TransportMemType &inputType, TransportMemType &outputType)
{
    if (workflowMode_ == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE) {
    ...
    HCCL_INFO("[AlltoallMesh][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d]",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

u64 AlltoallMesh::CalcLoopMaxCount(const u64 cclBuffSize, const u32 unitSize)
{
    u64 maxCountPerLoop = cclBuffSize / (topoAttr_.userRankSize * unitSize);
    return maxCountPerLoop;
}

bool AlltoallMesh::IsDataSplitForRdmaSdmaConcurrent(const u64 curSize)
{
...
HcclResult AlltoallMesh::KernelRun(const OpParam &param, ExecMem &execMem)
{
    u32 perDataSize = 0;
    CHK_RET(SalGetDataTypeSize(param.DataDes.dataType, perDataSize));
    CHK_PRT_RET(perDataSize == 0,
        HCCL_ERROR("[AlltoallMesh][KernelRun]errNo[0x%016llx] datatype[%d] is invalid",
    ...
    std::unique_ptr<AlgTemplateBase> algorithmTemplate = 
        AlgTemplateRegistry::Instance().GetAlgTemplate(
            TemplateType::TEMPLATE_ALLTOALL_MESH, dispatcher_);
    ...
    HCCL_INFO("[AlltoallMesh][KernelRun] Algorithm execution completed.");
    return HCCL_SUCCESS;
}

HcclResult AlltoallMesh::KernelRunInterServer(const OpParam &param, ExecMem &execMem)
{
    HCCL_INFO("[AlltoallMesh][KernelRunInterServer] Delegating to KernelRun");
    return KernelRun(param, execMem);
    return HCCL_SUCCESS;
}

HcclResult AlltoallMesh::KernelRunIntraServer(const OpParam &param, ExecMem &execMem)
{
    HCCL_INFO("[AlltoallMesh][KernelRunIntraServer] Delegating to KernelRun");
    return KernelRun(param, execMem);
    return HCCL_SUCCESS;
}

HcclResult AlltoallMesh::Orchestrate(const OpParam& param, AlgResourceResponse& algRes)
{
    HCCL_INFO("[AlltoallMesh][Orchestrate] Starting algorithm orchestration");
...
REGISTER_EXEC("AlltoallMeshExecutor", AlltoallMesh, AlltoallMesh);
