...
#include "alltoall_mesh.h"
#include "alg_template_register.h"

namespace hccl {
AlltoallMesh::AlltoallMesh(const HcclDispatcher dispatcher) : AlgTemplateBase(dispatcher)
{
}

AlltoallMesh::~AlltoallMesh()
{
}

// Communication primitives
HcclResult AlltoallMesh::TxVector(const LINK &link, const std::vector<Slice> &txSlices)
{
...
HcclResult AlltoallMesh::RxVector(const LINK &link, const std::vector<Slice> &rxSlices)
{
...
HcclResult AlltoallMesh::Tx(const LINK &link, const Slice &txSlice)
{
    DeviceMem srcMem = outputMem_.range(txSlice.offset, txSlice.size);
    HCCL_DEBUG("tx srcMem[%p] range[%llu] size[%llu] ", srcMem.ptr(), txSlice.offset, txSlice.size);
...
HcclResult AlltoallMesh::Rx(const LINK &link, const Slice &rxSlice)
{
    DeviceMem dstMem = outputMem_.range(rxSlice.offset, rxSlice.size);
    HCCL_DEBUG("rx dstMem[%p] range[%llu], size[%llu] ",  dstMem.ptr(),
        rxSlice.offset, rxSlice.size);
...
HcclResult AlltoallMesh::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK> &links)
{
    CHK_SMART_PTR_NULL(dispatcher_);
    CHK_PTR_NULL(stream_.ptr());
    HCCL_INFO("AlltoallMesh run_async rank[%u] ranksize[%u] inputMem[%p] outputMem[%p] count[%llu]", \
    ...
    if (links.size() < rankSize) {
        HCCL_ERROR("[AlltoallMesh][RunAsync]rank[%u] linkSize is less than rankSize", rank);
        return HCCL_E_INTERNAL;
    }

    u32 unitSize = DataUnitSize(dataType_);
    if (unitSize == 0) {
        HCCL_ERROR("[AlltoallMesh][RunAsync]unitSize is zero");
        return HCCL_E_INTERNAL;
    }
    ...
    CHK_RET(RunAlltoall(rank, rankSize, slices_, links));

    if (barrierSwitchOn_) {
        // For non-ring algorithms, barrier is handled in algorithm implementation
    }

    HCCL_INFO("AlltoallMesh finished: rank[%u] end", rank);
    return HCCL_SUCCESS;
}

// Core algorithm implementation
HcclResult AlltoallMesh::RunAlltoall(u32 rank, u32 rankSize, const std::vector<Slice> &outputSlices, const std::vector<LINK> &links)
{
    if (outputSlices.size() < rankSize) {
        HCCL_ERROR("[Run][Alltoall]rank[%u] OutputSlice Size is less than rank size", rank);
        return HCCL_E_INTERNAL;
    }
    ...
    // AllToAll Algorithm Implementation
    // DSL-driven all-to-all communication
    
    // Generic AllToAll: each rank sends to all other ranks
    for (u32 dstRank = 0; dstRank < rankSize; dstRank++) {
        if (dstRank != rank && dstRank < links.size()) {
            u64 chunkSize = sliceSize;
            DeviceMem srcMem = outputMem_.range(dstRank * chunkSize, chunkSize);
            
            CHK_RET(links[dstRank]->TxAck(stream_));
            CHK_RET(links[dstRank]->TxAsync(UserMemType::OUTPUT_MEM,
                dstRank * chunkSize + baseOffset_, srcMem.ptr(), chunkSize, stream_));
            CHK_RET(links[dstRank]->TxWaitDone(stream_));
        }
    }
...
REGISTER_TEMPLATE(TemplateType::TEMPLATE_ALLTOALL_MESH, AlltoallMesh);
}  // namespace hccl