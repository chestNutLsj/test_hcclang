...
def find_isomorphisms(topology, target_topology, limit=None, logging=False):
    ...
    if len(topology.switches) > 0:
        print('HCCL Warning: Topologies with switches are not supported. import hccl will be ignored.')
        return []

    if limit != None and limit <= 0:
        raise ValueError('HCCL error: limit was set improperly.')
    
    if topology.num_nodes() != target_topology.num_nodes():
        raise ValueError('HCCL error: target topology does not match with the given topology.')
